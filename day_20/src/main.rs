...
fn part_one(contents: String) {
    ...
    {
        let name_dest: HashMap<String, Vec<String>> = modules.iter().map(|(name, module)| {
            (name.clone(), module.destinations.clone())
        }).collect();

        name_dest.iter().for_each(|(name, destinations)| destinations.iter().for_each(|dest| {
...
impl Module {
    ...
    fn send_pulse(&mut self, sender: String, value: Pulse) -> Option<Vec<(Pulse, String)>> {
        match (&mut self.class, value) {
            (&mut ModuleType::FlipFlop(ref mut flip_flop), Pulse::Low) => {
            ...
            (&mut ModuleType::Conjunction(ref mut conjunction), _) => {
                ...
                if conjunction.inputs.iter().all(|(_, val)| *val == Pulse::High) {
                    Some(
                        self.destinations
                            .iter()
                            .map(|dest| (Pulse::Low, dest.clone()))
